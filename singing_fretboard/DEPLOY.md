...
- 改用「弦吟指板 v1.6_範圍卡片 Range Card」重新產生 `index.html`。
- 範圍選取（網頁版／平板）：在指板上按住左鍵拖曳即可框選，也可從指板外的空白處開始拖；點一下仍是新增音符，拖曳時不會輸入音符。Shift＋拖曳保留為快捷鍵。
- 範圍卡片常駐：未選取時為灰色虛線、按鈕不可點；選取後可「套入音階／清空／取消」。套入音階、空白指板、整組上色在有選取時只作用於範圍內。
- 整組上色新增「全部」按鈕（四格圖示）。
- 圓點標示移到起手模式下方（手機版不變）。
- 左上 logo 可點擊，開新分頁到 huanyinliu.com。
